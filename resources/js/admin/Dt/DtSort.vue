<template>
  <span class="cursor-pointer" @click="change()">
    <slot />
    <svg
...
<script>
export default {
  props: ["field", "direction", "orderBy"],
  methods: {
    change() {
      let url = {
        orderBy: this.field.id,
  ...
  computed: {
    isSorted() {
      return this.field.id === this.orderBy;
    },
    isAsc() {
      return this.isSorted && this.direction == "asc";
    },
    isDesc() {
      return this.isSorted && this.direction == "desc";
    }
  }
};
</script>
